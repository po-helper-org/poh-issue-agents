...
async def test_patch_marker_is_recorded_on_every_closing_run():
    ...
    Тест закрепляет форму вызова, а не прежнее правило: оно требовало «первым
    операндом, всегда» и отменено (#311, PR #313) — переносить `patched` между
    активациями нельзя, это само роняет реплей. Здесь вызов стоит в начале
    `_phase_on_close`, и менять его местоположение допустимо только вместе с
    разведением идентификаторов (#315): пока их два на одного, поведение обоих
    читателей задаётся тем, чей вызов случился раньше.

    Проверяется прогон, закрывшийся из `pr-review`, — фаза, где новый ход не
    нужен, и именно там маркер не попал бы в историю при другой форме вызова.
    """
