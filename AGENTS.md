...
У `e2e_live.py` три сценария, и они отвечают на разное:

| | что проверяет | по чему судит |
|---|---|---|
| `triage` | классификация и приоритет | метки Issue |
| `label-command` | запуск командой из метки и обратный ход меток | метки Issue |
| `develop` | стадию разработки целиком, с `--fix-round` — и круг правок | **файлы пул-реквеста** |

`develop` смотрит на PR, а не на метки, потому что у этой стадии «готово» и
состоявшаяся правка — разные вещи: см. таблицу отказов выше, строку про пустой
дифф. Он запускается прямо на очереди `developer`, минуя триаж и аналитику, и
занимает десятки минут. Правка стадии разработки без этого прогона считается
непроверенной.
