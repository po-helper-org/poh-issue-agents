...
Опасность не в порядке операндов, а в ВТОРОМ читателе того же идентификатора с
другим условием: первый сработавший вызов решает за весь прогон, и второй
читатель получит уже принятое решение. Нужен второй читатель — заводите ему
свой маркер либо сознательно принимайте, что оба открываются разом (так сделано
в `issue-lifecycle-merged-from-pr-open`: одна строка таблицы переходов, два
читателя, оба в одной задаче воркфлоу).
