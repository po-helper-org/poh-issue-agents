...
## bft-writer — генератор БФТ

- Источник: https://github.com/po-helper-org/poh-bft-writer (вложено на коммите
  `564c0ff`).
- Бизнес-функциональные требования в корпоративном формате: быстрый проход
  (письмо Цель / How to demo / Открытые вопросы / Границы / Документация) и
  канонический пайплайн из семи стадий с якорями на источники и нулевым
  допуском к выдумкам. Это тот самый навык, который вызывают команды `/bft` и
  `/bft-deep` этого сервиса — см. `docs/BFT.md`.
- Навыки: `bft-writer`, `bft-fast`, `bft-deep-swarm`, `bft-indexer`.
- Команды (`.claude/commands/`): `bft-index`, `bft-context-gen`, `bft-problem`,
  `bft-concept`, `bft-debate`, `bft-draft`, `bft-validate`, `bft-fast`,
  `bft-deep`, `bft-deliver`.
- **Расхождение с upstream — одно, помечено в самом файле.**
  `bft-context-gen.md` дополнен разделом «Постановка без JIRA»: он велит читать
  `artefacts/po-statement.md` как основной источник задачи. В контуре Issue
  трекера нет, и без этого стадия контекста искала бы эпик, которого не
  существует. Обновляя навык из upstream, раздел нужно восстановить.
- Confluence и JIRA не используются: подготовка прогона пишет в клон свой
  `bft-config.md` с пустыми `wiki_space` и `tracker_projects`. Гейт навыка
  «сухой прогон → явное „ок“ PO» в автономном прогоне пройти некому, поэтому
  публикация выключена, а результат уезжает веткой в GitHub.
