...
5. Каждый `claude -p` запускается неинтерактивно: `--print --permission-mode acceptEdits --dangerously-skip-permissions` (запись файлов без промптов в песочнице контейнера).
...
|----|------|------------|---------|-----------|
| R-01 | tool-calling `claude -p` через Anthropic-эндпоинт z.ai не работает (для GLM зафиксирована несовместимость OpenAI tool-calling, `worker/llm.py:28-30`) | Высокая | Высокое | Спайк-gate (4.3.1) до основной реализации; при провале пивот на концепт D (`worker/llm.py:41-53`) |
