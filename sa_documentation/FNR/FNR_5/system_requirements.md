...
**Статус: выполнено 19.08.2026.** Учётная запись не требуется — вердикт дебатов остаётся в силе. Отчёт и пять поправок к настоящему документу: `docs/spikes/2026-08-19-repowise-headless.md`.
...
2. Описать сервисы:
   a. MCP-эндпоинт workspace `contour` — `repowise mcp --transport streamable-http --host 0.0.0.0`, порт наружу не публикуется;
...
5. Собрать образ на базе `python:3.11-slim`, взяв за основу `../poh-infra/mcp/poh-org/images/Dockerfile.repowise`:
   a. `repowise` **точной версией `0.44.0` и не ниже** — в `0.36.0` у `repowise mcp` нет флага `--host`, эндпоинт слушает только loopback и по сети недостижим;
   b. пакет `git` — базовый образ его не несёт, и без него `repowise doctor` даёт `Git repository FAIL`, а история и blame недоступны;
   c. `git config --system --add safe.directory '*'` — контейнер работает от root, смонтированные клоны принадлежат другому пользователю.
6. Флаг `--host 0.0.0.0` обязателен для каждого сетевого сервиса: значение по умолчанию `127.0.0.1` делает публикацию порта бессмысленной.
...
1. Поднять обратный прокси как единственный публикуемый наружу сервис, обслуживающий выделенный домен.
   a. Переменная публичного домена — `REPOWISE_PUBLIC_HOST`. Имя `REPOWISE_HOST` **занято самим пакетом** под адрес привязки сокета (`repowise mcp --host` читает его по умолчанию), и домен в нём заставит эндпоинт слушать на несуществующем адресе.
...
6. Задать `REPOWISE_API_KEY` веб-сервису: при привязке к не-loopback адресу без него API отвечает `403` с текстом «Server is reachable from the network but REPOWISE_API_KEY is not set». Это собственный guard пакета и второй слой доступа помимо базовой аутентификации.
7. Проверку живости вести по `/health`: она открыта и ключа не требует.
...
| R-6 | **Подтверждён 19.08.2026.** Модель z.ai принята (`--provider openai --model glm-4.6`), **эмбеддинги отвергнуты** (`Unknown Model`, код 1211): векторный индекс пуст, семантический поиск недоступен | Достоверно | Среднее: полнотекстовый поиск работает, поиск по смыслу — нет | **Решено 19.08.2026: отдельный ключ у провайдера с эмбеддингами** (OpenAI, Gemini либо OpenRouter — поддерживаются пакетом). Следствие, принятое осознанно: текст кода всех репозиториев организации уходит третьей стороне; ключ заводится вторым, помимо z.ai. Структурный индекс от эмбеддера не зависит |
