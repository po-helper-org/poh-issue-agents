...
# Шаги стадии исполняет пакет. Здесь остаются их имена, чтобы воркфлоу и
# регистрация воркера продолжали писать привычное `activities.dev_prepare`.
#
# Переэкспорт — имён, а не поведения: внутренности стадии живут в
# `poh_developer.activities`, и подменять их в тестах нужно ТАМ. Подмена
# `activities._dev_paths` здесь не дошла бы до кода, который её читает.
#
# Модулем, а не именами: `build_mvp_plan` зовёт внутренность стадии
# (`_dev_paths`), и каталог прогона у плана и у стадии обязан быть ОДИН, иначе
# агент не найдёт положенный ему файл. Через модуль — чтобы подмена в тестах
# била в то же место, что и у самой стадии: связанное `from ... import _dev_paths`
# имя пережило бы подмену и молча считало бы каталог по-своему.
from poh_developer import activities as developer_stage
from poh_developer.activities import (  # noqa: F401
    capture_episode,
    collect_dev_followups,
    dev_announce,
    dev_announce_repair,
    dev_begin,
    dev_diagnose,
    dev_dispatch,
    dev_empty_run_reason,
    dev_followups,
    dev_prepare,
    dev_publish,
    dev_publish_partial,
...
# Чтение критерия приёмки переехало в `shared/howtodemo.py` — туда же, откуда его
# берёт мост к приёмщику (#301): у критерия два читателя, и пока функция жила
# здесь, второму она была недоступна. Имя оставлено алиасом: функция одна, менять
# три вызова ниже и полсотни проверок ради переименования не за чем.
_howtodemo_block = howtodemo.read
...
async def build_mvp_plan(issue: IssueInput, branch: str) -> bool:
    ...
    """
    _, clone_dir = developer_stage._dev_paths(issue)
