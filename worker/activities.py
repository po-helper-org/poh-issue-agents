...
def _claude_anthropic_creds(model: str | None = None) -> tuple[str, str, str | None]:
    ...
    Возвращает кортеж (token, base_url, model) — модель передаётся в claude -p
    через переменную окружения ANTHROPIC_MODEL или берётся из дефолта."""
    ...
    # Если модель не передана явно, пробуем взять из ANTHROPIC_MODEL
    if model is None:
...
def _run_claude(prompt: str, cwd: str, mcp_config: str | None = None, model: str | None = None) -> None:
    ...
    не увидела и артефакта не создана.
    
    `model` — опциональная модель для стадии. Если не передана, используется
    дефолтная модель из ANTHROPIC_MODEL или та, что задана в кредах.
    """
    ...
    # Подготавливаем окружение для claude -p
    env = {**os.environ, "ANTHROPIC_AUTH_TOKEN": token, "ANTHROPIC_BASE_URL": base}
    
    # Если модель задана, передаём её через переменную окружения
    if resolved_model:
...
async def run_fnr_stage(analyze: AnalyzeInput, stage_name: str) -> dict:
    ...
    # и лишние деньги.
    # Выбираем модель для данной стадии из конфигурации в llm.py
