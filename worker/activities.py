...
async def trigger_openhands_resolver(issue: IssueInput, root_issue: int | None = None, 
    ...
    Между постановкой и прогоном агента вызывается `build_mvp_plan` — план
    работ навыком writing-plans, файлом `.harness/plan.md` (Task 9, remainder
    после revert 80b3291: per-step деление на под-задачи откачено ревью, а
    план К ОБЫЧНОМУ ПРОГОНУ остался и подключается здесь). Место и порядок
    строгие, см. докстринг блока ниже; отказ построения плана — не отказ
    разработки.
    """
    ...
    # MVP: план работ — СТРОГО здесь, между готовым рабочим местом и стартом
    # агента, не раньше и не позже.
    #
    # Не раньше: `.harness/`, который читает и куда пишет `/plan-mvp`,
    # наполняет ТОЛЬКО `_dev_prepare` выше — до неё каталога не существует.
    # Прежняя попытка (Task 9, откачена review'ом, revert 80b3291) звала
    # планирование ДО подготовки; находка K2 того ревью — «холодный старт»:
    # стадия падала в каталоге, которого никто ещё не создал.
    #
    # Не позже: план — вход агента, а не отчёт по итогам его работы. Смысл
    # строить его раньше `_dev_run_agent`, а не параллельно и не после.
    #
    # Отказ НЕ роняет прогон и не идёт через отдельную попытку Temporal:
    # план — необязательный вход агента, а не результат стадии, и агент
    # УЖЕ СЕГОДНЯ, для каждого прогона, штатно работает без него (`PLAN` не
    # входит в `task_context.required()` намеренно). Топить весь прогон
    # разработки — самую дорогую стадию цикла, которая и без плана исправно
    # работала, — из-за упавшего необязательного шага значило бы разменивать
    # штатный путь на необязательное ускорение. По той же причине план не
    # оборачивается вторым вызовом `_run_with_heartbeat`: `build_mvp_plan`
    # уже бьёт heartbeat вокруг своего `claude -p` сама (см. её докстринг) —
    # обёртка вокруг обёртки истории не добавляет.
    #
    # Постановка (`task`, залогирована и записана строками выше) собрана
    # ДО этого места и ничего не обещает про план — она не читает и не
    # упоминает `.harness/plan.md`. Обещать агенту файл, которого могло не
    # получиться, значило бы дать ложный ориентир на упавшем построении;
    # агент сам увидит `.harness/plan.md`, если тот лёг, листингом каталога,
    # который ему и так велено читать (см. `_dev_prepare`).
    try:
        has_plan = await build_mvp_plan(issue, branch)
    except Exception as exc:  # noqa: BLE001 — план необязателен, разработка нет
        logger.warning(
            "Develop %s#%s: план работ не построен (%s) — агент продолжит "
            "без него, как и в любом прогоне без стадии планирования",
            issue.repo, issue.issue_number, exc)
    else:
        if has_plan:
            logger.info("Develop %s#%s: план работ построен (%s/%s)",
                        issue.repo, issue.issue_number,
                        task_context.DIR, task_context.PLAN)
        else:
            logger.warning(
                "Develop %s#%s: план работ пуст или не создан — агент "
                "продолжит без него, как и в любом прогоне без стадии "
                "планирования", issue.repo, issue.issue_number)
