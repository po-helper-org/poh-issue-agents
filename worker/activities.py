...
# --- MVP: воркфлоу MvpDelivery — шаги плана и под-задачи на время шага ---
#
# Активности этой секции вызывает `mvp_delivery.MvpDelivery` по СТРОКОВОМУ
# имени (см. докстринг модуля) — регистрация в `worker/worker.py` обязательна
# для каждой из четырёх, но AST-проверка `test_worker_wiring.py` её не
# ловит: та разбирает только `workflows.py` и только обращения вида
# `activities.<имя>`, а `mvp_delivery.py` ничего из этого не делает.

@activity.defn
async def mvp_read_plan(issue: IssueInput) -> list[dict]:
    """Строит план (навык writing-plans, `build_mvp_plan`) и разбирает его в
    шаги чистой функцией `plan_parse.parse` — одной активностью, а не двумя
    шагами воркфлоу: файл `.harness/plan.md`, который пишет `build_mvp_plan`,
    читается из того же рабочего каталога (`_dev_paths`), которому воркфлоу
    не видит — видит только активность.

    Честность рёбер проверяется здесь и только здесь: план входит в
    остальной контур единственный раз, ДО того как заведётся хоть одна
    под-задача. `decomposition.dependency_reason_missing` была готова и
    покрыта тестами, но её не звала ни одна задача плана — защита от
    выдуманной зависимости лежала мёртвым грузом. «Consumes: Task N» без
    названного предмета неотличимо от угаданного или сфабрикованного ребра, а
    развернуть такое ребро в под-задачу — значит заплатить задачей в
    трекере за доверие, ради которого вся эта декомпозиция и затевалась.

    Выбор при находке — отказ стадии (`InvalidPlan`), а не тихое исполнение
    без «плохого» ребра. Довод тот же, что у `decomposition.validate`:
    под-задача, единожды заведённая в GitHub, не отменяется одной кнопкой, а
    молчаливая правка плана перед исполнением спрятала бы от человека, что
    контур собирается делать не то, что написано в `plan.md`. Отказ стоит
    одного прогона, который заново инициирует человек; фабрикация стоит
    задачами в бэклоге.

    Повторная попытка своей же логики не предусмотрена (см. `_READ_PLAN_RETRY`
    в `mvp_delivery.MvpDelivery`): активность недетерминирована и стоит денег
    (`build_mvp_plan` — вызов модели до 15 минут) — повтор ПОСЛЕ отказа
    `RuntimeError`/`InvalidPlan`/`ValueError` инициирует человек, а не политика
    ретраев. Вторая попытка Temporal у той же активности остаётся — но только
    на случай, если стадию оборвали не эти три исключения, а авария
    инфраструктуры (воркер убит выкладкой посреди вызова claude), тот же приём,
    что у `run_fnr_stage`/`run_bft_stage` (`worker/workflows.py`).
    """
    branch = await _dev_resolve_branch(issue)
    ok = await build_mvp_plan(issue, branch)
    if not ok:
        # Молчаливое `[]` здесь замаскировало бы отказ навыка под легитимное
        # «план пуст»: воркфлоу читает пустой список как «делить нечего» и
        # тихо завершается с `None` — самый дорогой класс отказов (шаг
        # отработал, доложил успех, результата нет).
        raise RuntimeError(
            f"план MVP не построен: {task_context.PLAN} в {task_context.DIR} "
            f"отсутствует или пуст после навыка writing-plans "
            f"({issue.repo}#{issue.issue_number})")

    _, clone_dir = _dev_paths(issue)
    plan_path = Path(clone_dir) / task_context.DIR / task_context.PLAN
    items = plan_parse.parse(plan_path.read_text(encoding="utf-8"))

    dishonest = decomposition.dependency_reason_missing(items)
    if dishonest:
        raise decomposition.InvalidPlan(
            "план содержит зависимость без объяснения предмета — похоже на "
            f"выдуманное ребро (шаги: {', '.join(dishonest)}); поправьте "
            f"Consumes в {task_context.PLAN} и повторите")
    return items


@activity.defn
async def mvp_open_substep(issue: IssueInput, index: int, title: str) -> int:
    """Заводит под-задачу шага и привязывает её к родителю нативной связью
    GitHub (`create_issue` → `issue_node_id` → `link_sub_issue`).

    Метки `origin:agent` и `harness:step` — оба сразу, а не деталь оформления.
    Без `STEP` вебхук пропускает под-задачу мимо гейта Task 13 и поднимает на
    ней полноценный жизненный цикл с триажом: план родителя её уже разобрал,
    и такой цикл был бы разговором контура с самим собой, а вся экономия
    времени, ради которой заводится под-задача шага, потеряна. Закреплено
    тестом (`tests/test_mvp_activities.py`), не только этим комментарием.
    """
    body = (
        f"root-issue: #{issue.issue_number}\n\n"
        f"Родительская задача: #{issue.issue_number}\n\n"
        f"Шаг {index + 1} плана MVP: {title}\n\n"
        "Под-задача существует на время шага и закрывается вместе с ним."
    )
    number = await asyncio.to_thread(
        github_client.create_issue, issue.repo, title, body,
        labels=[labels.ORIGIN_AGENT, labels.STEP])
    node_id = await asyncio.to_thread(github_client.issue_node_id, issue.repo, number)
    await asyncio.to_thread(
        github_client.link_sub_issue, issue.repo, issue.issue_number, node_id)
    return number


@activity.defn
async def mvp_develop_step(issue: IssueInput, step_index: int) -> int:
    """Один шаг плана — существующим прогоном разработки.

    Переиспользует `trigger_openhands_resolver`: клон, прогон агента, тесты и
    публикация уже написаны, проверены и учитывают уроки боевых инцидентов
    (пустой прогон, служебные файлы в диффе, `.gitignore` целевого репозитория
    съедает контекст) — заводить вторую копию этой логики значило бы завести
    её без единого из этих уроков.

    `root_issue=issue.issue_number`: тот же приём, что и у подзадачи плана
    (ISSUE-113) — ветка и рабочий каталог общие на все шаги одного MVP
    (`develop.work_branch`/`_dev_paths` ключуются номером issue, а не индексом
    шага), поэтому шаги накапливаются на одной ветке и одном PR.

    Что из этого намеренно НЕ входит в задачу 9: сужение промпта агента до
    одного шага плана. `step_index` сегодня только для лога — агент видит
    весь `.harness/plan.md` и решает сам, как в обычном прогоне. Тесты
    воркфлоу (`tests/test_mvp_delivery.py`) полностью подменяют эту
    активность и не проверяют её тело: сужение промпта — отдельная задача.
    """
    logger.info("MvpDelivery %s#%s: разработка шага %d",
               issue.repo, issue.issue_number, step_index)
    number = await trigger_openhands_resolver(issue, root_issue=issue.issue_number)
    if number is None:
        # `dispatch`: результат приедет позже событием `pr-open` на чужой
        # стороне — несовместимо с тем, что MvpDelivery ждёт номер PR сразу
        # же, чтобы завести под-задачу следующего шага. Явный отказ вместо
        # того, чтобы `result_type=int` воркфлоу споткнулся о `None`.
        raise RuntimeError(
            f"MvpDelivery не поддерживает режим {develop.DISPATCH!r} "
            f"({issue.repo}#{issue.issue_number}): шаг плана ждёт номер PR "
            "сразу, а dispatch отдаёт его позже событием pr-open")
    return number


@activity.defn
async def mvp_close_substep(issue: IssueInput, number: int, index: int) -> None:
    """Закрывает под-задачу шага и отмечает шаг сделанным в теле родителя.

    Порядок — закрыть, потом отметить — намеренный: закрытие GitHub Issue
    идемпотентно (PATCH `state=closed` на уже закрытой задаче — не отказ), а
    отметка чеклиста ниже сама проверяет, не поставлена ли уже, так что
    повторный заход после сбоя между двумя шагами не плодит побочных
    эффектов ни там, ни там.
    """
    await asyncio.to_thread(github_client.close_issue, issue.repo, number)

    body = await asyncio.to_thread(
        github_client.get_issue_body, issue.repo, issue.issue_number)
    line = f"- [x] Шаг {index + 1} — #{number}"
    current = issue_blocks.read(body, issue_blocks.MVP_PLAN) or ""
    done = [ln for ln in current.splitlines() if ln.strip()]
    if line not in done:
        done.append(line)
    updated = issue_blocks.write(body, issue_blocks.MVP_PLAN, "\n".join(done))
    await asyncio.to_thread(
        github_client.update_issue_body, issue.repo, issue.issue_number, updated)
