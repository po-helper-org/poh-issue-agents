...
class IssueLifecycle:
    ...
    async def _phase_await_build(self, issue: IssueInput, deadlines) -> tuple | None:
        ...
        #
        # Открытый вопрос блокирует автостарт: задача обязана идти в обычную
        # парковку ниже и ждать сигнала, как и без автостарта. Условие не
        # трогает автостарт для задач, у которых критерий уже есть, — там
        # `self._open_question` пуст, и ветка ведёт себя как раньше.
        #
        # `workflow.patched` обязателен: у прогонов, уже застрявших в этом
        # витке СТАРЫМ кодом (указатель на вопрос уже в истории, а таймер или
        # сигнал этой парковки в истории при этом нет), новая ветка здесь
        # означала бы для них другую последовательность команд на реплее —
        # `_park`/`_wait_for_signal` вместо продолжения автостарта. Короткое
        # замыкание `and` — тем же приёмом, что и `issue-lifecycle-plan-
        # member-waits-for-parent` ниже: `workflow.patched` зовётся, только
        # когда вопрос уже открыт, и это делает точку первого вызова для
        # данного прогона предсказуемой и воспроизводимой при реплее.
        autostart_blocked_by_open_question = (
            bool(self._open_question)
        ...
        if (deadlines.develop_autostart and not self._plan_member
                and not autostart_blocked_by_open_question):
            return await self._start_development(issue)
