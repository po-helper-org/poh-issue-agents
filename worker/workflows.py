...
class IssueLifecycle:
    ...
    async def _phase_on_close(self) -> tuple[str, str]:
        ...
        Вызов стоит до ранних возвратов — так он был написан под прежнюю
        редакцию правила 1, требовавшую «первым операндом, всегда». Правило
        отменено (#311): опасность не в порядке операндов, а в двух читателях
        одного идентификатора, решающих РАЗНОЕ. Здесь это и вышло — второй
        читатель в `_agent_event`, и его ранний вызов на реплее закрывает эту
        ветку для старых прогонов (#315). Форму вызова трогать без той правки
        нельзя: перенос `patched` между активациями сам по себе роняет реплей.
        """
    ...
    async def _agent_event(self, event: AgentEvent) -> tuple:
        ...
        # Маркер тот же, что у ветки закрытия: строка таблицы одна. Решения,
        # однако, РАЗНЫЕ — принять доклад и записать исход закрытия, — а
        # запомненное значение первого вызова достаётся второму. Для прогонов,
        # начатых до выкладки, это закрывает ветку закрытия (#315, там же и
        # разведение идентификаторов).
        merged_from_pr_open = workflow.patched(
            "issue-lifecycle-merged-from-pr-open")
