...
| Выбор провайдера по репозиторию | **работает** — `worker/forge.py`, переменная `GITLAB_REPOS` |
| Выкладка на стенд | **нет** — стенд собран с пином на прежний SHA |
| Вебхук на проекте GitLab | **нет** — ставится после выкладки |
| Пакет `poh-forge` | **нет** — вынос отложен, драйверы живут в репозитории |
...
**Практическое следствие.** Код работает с обоими трекерами одновременно и проверен против живых `gitlab.com` и `github.com`. Осталось развернуть: стенд собран с пином на прежний SHA, поэтому эндпоинта там ещё нет, и вебхук в проекте GitLab настраивать рано — доставки получили бы 404.

Порядок остатка: выложить на стенд → задать `GITLAB_TOKEN`, `GITLAB_REPOS`, `GITLAB_WEBHOOK_SECRET`, `GITLAB_BOT_LOGIN` → шаг 5 ниже.
...
### Шаг 3.5. Отнести репозиторий к GitLab

Allowlist контура (`ISSUE_AGENT_REPOS`) говорит, **какие** репозитории обслуживать. Он не говорит, **у какого провайдера** они живут — это отдельная переменная:

```
GITLAB_REPOS=poh-harness/*
```

Формат тот же. Всё, что под неё попадает, уходит в клиент GitLab; остальное остаётся у GitHub.

**Пустая переменная означает GitHub, а не «всё подряд».** Проверка допуска на пустом списке разрешает всё, и без явного умолчания включение переменной было бы не нужно — весь трафик уехал бы в GitLab молча.

Проверить, как контур развёл репозитории:

```bash
GITLAB_REPOS='poh-harness/*' python3 -c "
import sys; sys.path.insert(0,'worker'); sys.path.insert(0,'.')
import forge
for r in ('poh-harness/threads-harness', 'po-helper-org/poh-demo-checkout'):
    print(f'{r:42} -> {forge.provider_for(r)}')
"
```
