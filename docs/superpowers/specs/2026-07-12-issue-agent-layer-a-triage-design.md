...
- Механизм бэкфилла: старт `IssueLifecycle`-workflow для каждого уже открытого
  Issue (webhook такие события не присылает), с не-интерактивным batch-режимом
  (`interactive=False`), чтобы VAGUE-issue эскалировались, а не зависали.
...
5. **VAGUE зависает в бэкфилле.** Цикл уточнений ждёт `user_comment` через
   `_wait_for_signal()` без таймаута. В бэкфилле человек не отвечает на 39
   issue → VAGUE-issue висят *running* вечно, пункт 1 критерия ложно не
   выполнен. Нужен не-интерактивный batch-режим (см. Решение 1).
6. **Маппинг автора.** `gh --json author` не отдаёт `type`; без деривации из
   `is_bot` бот-предфильтр не сработает (см. Решение 1).
...
- **Маппинг автора:** `gh --json author` отдаёт `{login, is_bot}`, а не `type`.
  Бэкфилл выставляет `author_login = author.login` и
  `author_type = "Bot" if author.is_bot else "User"` — иначе бот-предфильтр
  (`author_type == "Bot"`) промахнётся.
- **Batch-режим (не-интерактивный).** В бэкфилле нет человека, отвечающего на
  уточнения. Добавляем в `IssueInput` поле `interactive: bool` (по умолчанию
  `True` для webhook-пути Слоя B). Бэкфилл ставит `interactive=False`. В
  workflow: при `gate.status == "VAGUE"` и `interactive is False` — сразу
  `escalate_to_human` + `return`, **без** входа в цикл уточнений с
  бесконечным `_wait_for_signal()`. Иначе VAGUE-issue висели бы *running*
  вечно, и критерий «прошлась по каждому» был бы ложно не выполнен.
...
scripts/backfill.py
  → start_workflow(IssueLifecycle, IssueInput) для каждого open Issue
      ...
      → intake_gate (GLM-air)        (SPAM → закрыть; VAGUE → batch: escalate+стоп,
                                        webhook: цикл уточнений)
...
- **Минимальный error-guard (в объёме Слоя A).** Workflow оборачивает
  автономную последовательность в try/except: при падении activity после
  исчерпания ретраев — `escalate_to_human`-подобный шаг (лейбл `advisor:error`
  + короткий комментарий «автообработка не удалась, ожидай ручного разбора»),
  затем `return`. Без этого на боевом прогоне 39 упавший issue остаётся вовсе
  без обратной связи — «прошлась по каждому» молча ложна. Расширенные сценарии
  (недоступность z.ai, override, edited) — в A-hardening.
- `DRY_RUN` исключает главный риск необратимости (массовое автозакрытие); при
  `DRY_RUN` error-guard тоже логирует, а не постит.
