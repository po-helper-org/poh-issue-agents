...
**Goal:** Оживить 28 прогонов `IssueLifecycle`, застрявших на ошибке недетерминизма, и завести проверку, которая ловит этот класс до мержа, а не через двое суток на проде.
...
- **Масштаб:** 28 из 124 живых `IssueLifecycle` мертвы. Список: `poh-issue-agents` #214–#240 (сплошным блоком), #112, `poh-demo-checkout` #29.
...
падает: правка ac625e7 развела код с историей 28 прогонов.
...
Проиграны все 28 застрявших прогонов: расхождение было одно.
...
28 прогонов `IssueLifecycle` из 124 живых не выполняют задачи воркфлоу. Сигналы к ним приходят и умирают, метки и фазы остаются прежними — снаружи Issue выглядит живым.
...
Проиграны все 28 застрявших историй против исправленного кода — падений ноль, расхождение было одно.
...
и 28 прогонов из 124 встали намертво. Ни один тест этого не увидел — обычные
