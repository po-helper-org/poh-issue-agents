...
2. `claude -p` запускается только с `--dangerously-skip-permissions` (в спеке 4.3.2.4 NFR 5 упомянут ещё `--permission-mode acceptEdits`): флаг пропуска разрешений его перекрывает, дублировать не нужно.
