...
### Незакрытые прогоны — главный риск выкладки

`IssueLifecycle` живёт неделями: один прогон на задачу, подолгу стоит на
ожидании сигнала. Передеплой не ждёт, пока они закончатся, — новый воркер
переигрывает их истории против нового кода. Если код изменил **решение**
воркфлоу (порядок активностей, вид ожидания, длительность таймера), порядок
разойдётся с записанной историей, и Temporal откажется продолжать:
...
Такой прогон **не падает** — он остаётся в `Running` и бесконечно повторяет
workflow task. Снаружи это неотличимо от «задача просто стоит»: Issue перестаёт
отвечать на сигналы, и человек об этом не узнаёт. Ретрай не лечит, правка кода
вперёд — тоже. Возвращает в строй только откат воркера на прежний образ.

**Перед пересборкой посчитай, сколько прогонов под ударом:**

```bash
make deploy-check                      # сколько их и какие старейшие
make deploy-check ARGS=--replay        # кто именно сломается
make deploy-check-stack                # то же изнутри контейнера (стек full)
```

Адрес кластера сторож берёт из `TEMPORAL_ADDRESS` в `.env`. В конфигурации
**full** порт 7233 наружу намеренно не публикуется, поэтому с хоста туда не
дотянуться — там работает `make deploy-check-stack`, запускающий ту же проверку
внутри воркера.

`--replay` тянет истории живых прогонов прямо из Temporal и проигрывает их
против текущего кода — то же, что делает `scripts/replay_histories.py` по
снятому корпусу, но без ssh и tar. Код возврата 1 означает «под ударом есть
прогоны», так что цель годится и как гейт в CI.

`make up` и `make up-local` печатают ту же цифру перед сборкой, но не блокируют
её (`--warn-only`): решение выкладывать — за человеком. У `make up-full` этого
шага нет — оттуда до Temporal не дотянуться, и вечное «недоступен» перед каждой
сборкой приучало бы не читать вывод сторожа.

Правило, которое снимает риск в источнике, — `workflow.patched(...)` на каждое
изменение решения, заведённый **до** выкладки. Задним числом он не лечит, а
ломает: подробности и таблица действующих маркеров — в
[`AGENTS.md`](../AGENTS.md), правило 1. Правка тела активности, её ретраев и
меток безопасна — их в истории нет.
...
**Задача перестала отвечать после выкладки.** Скорее всего прогон разошёлся с
историей. В логах воркера — строка `НЕДЕТЕРМИНИЗМ: прогон разошёлся с историей`
с `run_id` и причиной, в Sentry — событие с fingerprint `nondeterminism` (одно
на все пострадавшие прогоны: причина у них общая). Строку ставит наблюдатель
`shared/nondeterminism.py`; само ядро Temporal пишет свою версию отказа
(`[TMPRL1100] …`) в stderr мимо `logging`, так что в Sentry она не попадает —
искать надо именно строку `НЕДЕТЕРМИНИЗМ:`. Проверить масштаб — `make
deploy-check ARGS=--replay`. Лечение — откат воркера на прежний образ; после
отката завести `workflow.patched(...)` на изменение и выложить заново.
