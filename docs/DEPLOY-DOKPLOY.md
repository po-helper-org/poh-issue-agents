...
`make up` и `make up-full` печатают ту же цифру перед сборкой, но не блокируют
её (`--warn-only`): решение выкладывать — за человеком.
...
с `run_id`, в Sentry — событие с fingerprint `nondeterminism` (одно на все
пострадавшие прогоны: причина у них общая). Проверить масштаб — `make
