...
него диагностика сводится к чтению логов. Хост-порт он не публикует намеренно:
на Dokploy-хосте порт 8080 обычно уже занят (Traefik/панель), да и публиковать
порты руками здесь не принято. Плюс у самого интерфейса нет входа по паролю, а
видно в нём тела всех Issue и содержимое артефактов — торчать наружу как есть
он не должен.
...
2. Traefik-middleware с basic-auth. В Dokploy это делается через
   **Advanced → Middlewares** (или ярлыком `traefik.http.middlewares.*.basicauth`).
   Без этого шага интерфейс окажется открыт всему интернету — не выкладывай
   домен, пока basic-auth не включён.

Альтернатива без домена — SSH-туннель прямо в контейнер:

```bash
# на сервере узнать имя контейнера temporal-ui, затем
ssh -N -L 8080:<container-name>:8080 user@server
```

где `<container-name>` — имя из `docker ps` (вида
`...-temporal-ui-1`), резолвится в сети Docker. Затем открыть
`http://localhost:8080`.
