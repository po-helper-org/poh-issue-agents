...
TRANSITIONS: dict[str, tuple[Transition, ...]] = {
    ...
    READY_FOR_DEV: (
        ...
        # Тот же прогон за один шаг может и сорваться. Без этого перехода
        # обработчик отказа падал сам — `InvalidTransition` подменял настоящую
        # причину, и цикл крутил её вместо того, чтобы доложить и остановиться.
        Transition(FAILED, AGENT, "прогон разработки сорвался"),
    ...
    FAILED: (
        ...
        # Сорваться могла именно аналитика — и тогда `/analyze` не «новая
        # стадия», а второй заход на ту же. Без этого перехода команда
        # выполнялась, артефакты появлялись, а фаза оставалась `failed`: задача
        # навсегда стояла в очереди к человеку с готовыми требованиями на руках.
        Transition(BUSINESS_ANALYSIS, HUMAN, "/analyze — повторный заход на анализ"),
