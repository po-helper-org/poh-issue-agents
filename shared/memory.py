...
`shared/repowise.py` и `poh_developer/develop.py`: он вызывается из подготовки каталога
...
CONTROL_MOD_ENV = "MEMORY_CONTROL_MOD"
"""Каждая N-я задача идёт БЕЗ подсыпанных правил — контрольная выборка.

Без неё вопрос «слой не мешает?» неотвечаем в принципе. Доля дошедших до
пул-реквеста сама по себе не говорит ничего: агент, честно сообщивший «работа
уже сделана», пул-реквеста не открывает и правильно делает. Обвиняет только
РАЗНИЦА между выборкой с правилами и выборкой без них — а взять вторую неоткуда,
если правила подсыпаются всегда.

Пусто или ноль — контрольной выборки нет, правила идут в каждую итерацию. Это
умолчание: половина прогонов без накопленного опыта организации — цена, которую
платят осознанно и на время проверки, а не постоянно.

Деление ДЕТЕРМИНИРОВАННОЕ, по номеру задачи. Не случайное: повтор той же задачи
обязан попадать в ту же выборку, иначе сравнение сравнивает разные задачи.
"""


def control_arm(issue_number: int) -> bool:
    """Идёт ли эта итерация контрольной — без правил организации.

    Выключенный слой контрольных итераций не даёт вовсе: записывать их некуда,
    и «без правил» тогда означало бы просто «слоя нет».
    """
    if not enabled():
        return False
    raw = os.environ.get(CONTROL_MOD_ENV, "").strip()
    try:
        mod = int(raw)
    except ValueError:
        return False
    return mod > 1 and issue_number % mod == 0
