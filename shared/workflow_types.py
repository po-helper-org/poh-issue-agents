...
class LifecycleState:
    ...
    rework_rounds: int = 0
    # Идентификатор открытого вопроса к человеку (гейт критерия приёмки,
    # задача 8). Переносится по той же причине, что и остальное состояние
    # выше: continue-as-new не должен терять указатель на вопрос, который уже
    # объявлен человеку, — иначе `/harness-answer` после перезапуска цикла
    # находил бы «вопросов нет», хотя вопрос всё ещё висит в теле Issue.
    open_question: str = ""
    # Отказ гейта критерия приёмки уже показан хотя бы раз в ЭТОЙ серии
    # подряд идущих отказов (см. `_criterion_gate_notified` в
    # `_start_development`, worker/workflows.py — там же дедупликация «одно
    # сообщение на серию»). Переносится через continue-as-new — ревью
    # (находка 2) разобрало механику: порог длины истории проверяется ПОСЛЕ
    # КАЖДОГО перехода фазы, включая холостой «остались на месте после
    # отказа», а очередь сигналов сразу после разбора одного сигнала почти
    # всегда пуста. Для задачи, уже прошедшей триаж и застрявшей на этом
    # гейте, — ровно целевой сценарий этой правки — порог набирается быстро:
    # это НЕ редкое стечение обстоятельств, а обычный ход событий. Без
    # переноса каждый перезапуск обнулял бы флаг, и каждый повторный отказ
    # после него получал бы своё сообщение — обещание «одно на серию»
    # переставало бы выполняться именно там, ради чего писалось.
    criterion_gate_notified: bool = False
    # Гейт критерия приёмки не пропустил задачу дальше на последнем заходе
    # (третий круг финального ревью, находка G1). Переносится через
    # continue-as-new по той же причине, что и `open_question` рядом:
    # перезапуск не должен снимать защиту автостарта (`_phase_await_build` в
    # `worker/workflows.py`) с задачи, застрявшей на этом гейте, — иначе
    # каждый continue-as-new открывал бы автостарту новую лазейку в тот же
    # горячий виток, ради устранения которого флаг и заводился. См. докстринг
    # `autostart_blocked_by_gate_stall` в `_phase_await_build` за тем, почему
    # этот признак — а не просто непустой `open_question` — единственно
    # верный: гейт может остановить проход и без единого открытого вопроса
    # (отказ чтения критерия, отказ постановки вопроса).
    acceptance_gate_stalled: bool = False
