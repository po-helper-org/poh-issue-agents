...
# --- Журнал вызовов: чем занимался прогон ---
#
# Артефакты показывают РЕЗУЛЬТАТ, но не путь к нему: по `concept.md` не видно,
# что стадия делала три захода и на чём её оборвало. Журнал пишется рядом с
# артефактами и уезжает в ту же ветку — восстановить контекст можно из
# репозитория, не поднимая ни рабочий каталог, ни внешний сервис.
#
# Промпты целиком сюда НЕ кладутся: у стадии БФТ системная часть под сто
# килобайт (инструкция команды плюс ресурсы скилла), и в ветке она стала бы
# мусором, который никто не читает. Записывается то, чего нет в артефактах:
# какая стадия, каким исполнителем, сколько это стоило и чем кончилось.
