...
def exhausted_comment(max_rounds_: int) -> str:
    return (
        f"⚠️ Доведение остановлено: пройдено {max_rounds_} кругов правок, а "
        "замечания продолжают появляться.\n\n"
        "Дальше нужен человек. Столько кругов — это уже не придирки ревьюера: "
        "так выглядит проблема в самом коде или в том, как устроен репозиторий, "
        "и лечится она разбором, а не следующим кругом. Что именно осталось — "
        "в последнем комментарии ревьюера."
    )
