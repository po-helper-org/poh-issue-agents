...
| **Develop — передача задачи агенту разработки**: по `build-me` цикл диспатчит `openhands-resolver.yml` в репозитории-цели, паркуется в `in-development` и ждёт события `pr-open` | ✅ Работает | метка `build-me` на Issue после аналитики |
...
Код пишет внешний агент — **OpenHands в headless-режиме на раннере GitHub
Actions** репозитория-цели (`.github/workflows/openhands-resolver.yml`). Своё
окружение, свой sandboxing, свой релизный цикл; в этот compose он не
втаскивается. Граница описана в `shared/develop.py`, запуск — в активности
`trigger_openhands_resolver`.

**Запуск — `workflow_dispatch`, а не метка-триггер.** Метка — состояние, а не
...
**Edge-кейсы агент не чинит по дороге** — выносит отдельными SubIssue с
