...
|-------------|--------|-------------|
| **Layer A — автономный триаж Issue**: предфильтры → intake-gate (с циклом уточнений) → 4-way классификация + advisor-ответ → duplicate-check (только метка) → приоритет по формуле | ✅ Работает, прогнан вживую по реальному бэклогу (64/67 Issue размечено, 0 ошибочных закрытий) | `make dry-run` / `make backfill-one issue=N` / команда `/triage` или метка `run:triage` |
...
| **Запуск команд метками** (`run:analyze` / `run:estimate` / `run:triage` / `run:bft` / `run:bft-deep` → прогон → `done:*` либо `failed:*`) — управление с телефона в два тапа и индикатор состояния в списке Issue | ✅ Работает | метка `run:<команда>` на Issue |
