...
| **Гейт критерия приёмки**: разработка не начинается, пока в задаче нет прочитанного критерия готовности. Контур предлагает 2–3 варианта и ждёт ответа человека; ответ ложится в тело Issue блоком решений и больше не переспрашивается | ✅ Работает, проверено вживую на `poh-demo-checkout#166` и `#167` | само, перед стартом разработки; ответ — `/harness-answer <текст>` |
| **`/harness-answer` — ответ человека на любой вопрос контура**: один способ ответа на все поводы (критерий приёмки, границы MVP, выбор из вариантов). Дорогих стадий не запускает, меток прогона не ставит | ✅ Работает | комментарий `/harness-answer <текст>` в Issue |
| **Разработка отличает свою поломку от чужой и чинит свою**: красный прогон тестов разбирается — базовая линия на чистом дереве, перепроверка на мигание, разница множеств. Чужая краснота не мешает открыть PR, своя поломка идёт агенту на повторный заход | ⚙️ Смержено, проверяется живыми прогонами | само, на красных тестах; число заходов — `DEVELOP_REPAIR_ROUNDS`, умолчание 1 |
| **Сорванный прогон разработки оставляет материал для разбора**: отказ любого шага после агента даёт ветку и **черновой** PR с тем, что он успел написать, плюс комментарий с исходной причиной. Ревью на черновик не тратится | ✅ Работает, проверено вживую на `poh-demo-checkout#167` (черновик через 17 минут после заведения задачи) | само, на отказе разработки |
...
| **Приёмка по HowToDemo**: агент фиксирует сценарий приёмки указателем с хэшем (тело Issue либо первое письмо БФТ), переводит шаги в план, поднимает эфемерный стенд из ветки PR, проходит шаги и публикует отчёт «Что готово / Что не соответствует» с уликами. Вердикт считает код, не модель | ⚙️ Механизм готов (`po-helper-org/poh-howtodemo-agent`), вживую ещё не прогонялся | комментарий `/howtodemo` либо метка `run:howtodemo` — см. [`docs/HOWTODEMO.md`](docs/HOWTODEMO.md) |
...
- **Круг правок даёт агенту ровно один заход.** Не починил со второй попытки —
  задача уходит человеку. Число заходов поднимается `DEVELOP_REPAIR_ROUNDS`, но
  статистики, сколько починок удаётся, пока нет.
- **Отчёт JUnit XML нужно включить в `DEVELOP_TEST_COMMAND` целевого прогона.**
  Без него разбор красноты не работает и контур ведёт себя как прежде — молча.
- ~~**Прогон разработки кончается человеком на ЛЮБЫХ красных тестах.** `dev_tests`
  знает только код возврата и не отличает «сломал агент» от «было сломано до
  него». Живой случай: `main` демо-репозитория красный с 2026-09-01 из-за
  истёкшего промокода в тесте, и два прогона подряд (`#166`, `#167`) списаны в
  отказ, хотя агент ничего не ломал. CI это не ловит: тест покраснел по
  календарю, без коммита, а последний прогон на `main` был раньше. Замысел
  разбора — `docs/superpowers/specs/2026-09-02-dev-repair-loop-design.md`, не
  реализовано.~~ **Сделано 3 сентября** — см. раздел «Красный прогон
  разбирается» выше.
- ~~**Круга правок по своей поломке нет.**~~ **Сделано 3 сентября.**
- **CI не собирает образ.** `.github/workflows/tests.yml` гоняет только
  `pytest`; зелёный PR не доказывает, что контейнер соберётся и запустится.
  Сломанная сборка обнаруживается уже на стенде.
- **Ревью тратится на черновики там, где у репозитория свой воркфлоу
  PR-Agent.** Свипер черновики пропускает (правка в `po-helper-org/poh-pr-agents`),
  но Actions-воркфлоу целевого репозитория реагирует на `opened` независимо —
  проверено на `poh-demo-checkout#168`.
...
с `Closes #issue`. Если агент по дороге нашёл edge-кейсы (`collect_dev_followups`,
файл `.followups.md` в рабочем каталоге), новых Issue и комментариев это не
заводит: находки ложатся строками в секцию GROW тела этого же Issue и ждут
гейта приёмки.
...
- **Находки агента — файлом.** `gh` и GITHUB_TOKEN раннеру не дают: он исполняет
  ...
  `## <кратко>` на находку), а строками в секцию GROW тела родителя их
  дописывает воркер своим токеном (`collect_dev_followups`) — без новых Issue
  и без сети, если находок не было. Потолок — `develop.MAX_FOLLOWUPS`.
...
**Edge-кейсы агент не чинит по дороге** — `collect_dev_followups` дописывает
их строками в секцию GROW тела родителя, а не заводит по ним Issue сама.
Находка ждёт там гейта приёмки; отдельный Issue по ней заведёт человек, если
решит. Поэтому веера «каждый PR рождает Issue, каждый Issue — PR» не
получается: до новой задачи дело доходит только решением человека, а не
автоматически.

#### Гейт критерия приёмки — разработка не стартует вслепую

Перед запуском агента контур проверяет, есть ли в задаче **прочитанный критерий
готовности**: блок HowToDemo в теле Issue либо заголовок «Как принимаем» и его
варианты. Нет критерия — разработка не начинается: контур публикует 2–3
кандидата в формате «было — стало» и паркуется.

Человек отвечает командой `/harness-answer <текст>`. Ответ попадает в тело
Issue двумя блоками — журналом решений и самим сценарием приёмки, — и на
следующем заходе гейт его читает, а не спрашивает заново. Перезапуск прогона
вопрос не повторяет.

Гейт держит и автостарт: при `DEVELOP_AUTOSTART=1` задача без критерия не
уезжает в работу, а ждёт ответа. Прежде автостарт звал разработку на каждом
обороте цикла независимо от гейта — это давало горячий виток с вызовом модели
на каждой итерации, в котором ответ человека не читался вовсе.

#### Красный прогон разбирается, а не списывается в отказ

Раньше `dev_tests` знал только код возврата: любой красный набор означал отказ
разработки и человека. На `poh-demo-checkout` это дважды подряд списало работу
агента, который **ничего не ломал** — `main` репозитория красный с 1 сентября
из-за истёкшего промокода в тесте.

Теперь красный исход разбирается:

1. базовая линия снимается на **отдельном чистом дереве** от того же коммита
   (`git worktree`, никогда не `git stash` — сорванный `stash pop` уничтожил бы
   работу агента);
2. подозрительные падения перепроверяются повторным прогоном — упавшее один раз
   из двух считается мигающим и агенту не вменяется;
3. свои поломки = устойчиво упавшие минус упавшие до правки.

Дальше по исходу: своих поломок нет — прогон **удался**, открывается обычный PR
с оговоркой про чужую красноту в теле. Свои есть — агент идёт на повторный
заход, получая **только свои** падения (увидев чужие, он пошёл бы чинить чужой
код, и правка приехала бы в PR под видом решения задачи).

Заход один. Не починил — отказ, черновик, человек.

**Зелёный прогон не платит ничего:** всё дополнительное гоняется только на
красном.

**Имена упавших тестов берутся из отчёта JUnit XML**, а не из текста вывода:
Maven и Gradle пишут его сами, pytest по `--junit-xml`, `node --test` по
`--test-reporter=junit`. Отчёта нет или он не разобрался — контур ведёт себя
как прежде, любой красный считается отказом. Незнакомый стек ничего не ломает,
он просто не получает ни послабления, ни починки.

#### Сорванный прогон оставляет черновик, а не пустоту

Отказ **любого** шага после старта агента — сбор находок, тесты, сама
публикация — выкладывает то, что агент успел написать: ветка
`feature/<N>-openhands` и **черновой** PR с заголовком `СОРВАЛОСЬ feat(#N)`.
Комментарий называет прогон сорванным и несёт исходную причину.

Почему черновик, а не обычный PR: работа заведомо негодная — прогон не дошёл до
конца. Обычный PR выглядел бы кандидатом на слияние и съел бы бюджет ревью на
то, что контур сам признал негодным.

Три правила, без которых механизм вреден:

- **Отказ выкладки не подменяет исходную причину.** Агент упал по лимиту
  модели, а спасение потом не прошло — человеку сообщается про лимит, а про
  неудачу выкладки отдельной строкой.
- **Признак «есть что спасать» — не вид отказа, а наличие изменений.** Флаг
  «агент запущен» ставится ДО его старта: упавший на середине оставляет в
  дереве ровно то, ради чего это и делается. Пустое дерево отсекает сама
  выкладка.
- **Фаза остаётся `failed`.** Черновик не означает продвижения: задача в
  разборе у человека.

Повторный прогон обновляет прежний черновик, а не заводит второй.
...
`github_client.review_text` уже читал формальные ревью вместе с построчными
замечаниями — не хватало не чтения, а побудки: до 3 сентября вебхук на события
ревью не был подписан, и замечание человека не запускало ничего. Контур при
этом дважды объявлял «PR готов к слиянию», не увидев `CHANGES_REQUESTED`
(#302) — эта фраза теперь говорится не по факту «агент не нашёл предмета», а
по факту `github_client.changes_requested()`: состояние ревью считается по
ПОСЛЕДНЕЙ записи каждого автора, и старое `CHANGES_REQUESTED` не блокирует,
если тот же человек потом одобрил.
...
- `docs/GITLAB.md` — подключение проекта GitLab: токены и их ограничения,
  метки, allowlist, вебхук, отличия API от GitHub и раздел о том, что из
  поддержки GitLab уже работает, а чего в коде ещё нет.
