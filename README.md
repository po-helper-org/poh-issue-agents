...
Тесты: **331 тест**, покрытие **85%**. Локально — `make test` (после `make setup`
...
как запустить прогон заново, а сессия завершается — иначе забытый Issue держал бы
воркфлоу вечно, и список открытых прогонов рос бы монотонно.
