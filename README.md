...
Тесты: **653 теста**, покрытие держится порогом. Локально — `make test` (после `make setup`
...
отсюда два режима (`DEVELOP_MODE`, граница в `shared/develop.py`, запуск в
активности `trigger_openhands_resolver`):
...
Активность проходит путь целиком за один шаг — клон, постановка, агент, находки,
тесты, пуш, PR, — поэтому промежуточной фазы `in-development` у неё нет: ждать
нечего, результат уже здесь.
